...
const AptRisingRankMain = ({ item }) => {
  ...
  if (item) {
    item.forEach(function (item) {
      item.forEach(function (item2) {
        if (item2.년 === 2022) {
          apartNameAndPriceComposition2022 =
            "부산광역시" +
            item2.도로명 +
            item2.도로명건물본번호코드 +
            " " +
          ...
          apartNameAndPriceComposition2023 =
            "부산광역시" +
            item2.도로명 +
            item2.도로명건물본번호코드 +
            " " +
  ...
  let AptRisingNameAndPrice = resultArray2023
    .map(function (item1) {
      ...
      if (matchingItem) {
        let diffPercent = Number(
          (((item1.value - matchingItem.value) / item1.value) * 100).toFixed(2)
        );
        return {
          name: item1.name,
        ...
        idx <= 4 ? (
          <div className="apt_rankingRisinglist">
            ...
            <ul>
              <li>
                {item.value < 0 ? (
                  <p className="decrease">하락</p>
                ) : (
                  <p className="increase">상승</p>
                )}
              </li>
              <li>
